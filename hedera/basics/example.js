...
async function main() {
    ...
    client.setOperator(
        ...
        PrivateKey.fromString(process.env.OPERATOR_PVKEY)
    );
...
async function getEventsFromRecord(contractId) {
    ...
    const newMessage = '{"asset_id": "xxxxx", "ops": "{...}", "verse": "5"}';
    ...
    axios.get(url)
        .then(function (response) {
            ...
            jsonResponse.logs.forEach(log => {
                ...
                console.log(`Mirror event(s): from '${AccountId.fromSolidityAddress(event.from).toString()}' update to '${event.message}'`);
            });
        })
        .catch(function (err) {
            console.error(err);
